...
public final class UInt256 {
  ...
  private static final int N_BYTES_PER_LIMB_LOG = 2;
  ...
  // Helper method to read 4 bytes as big-endian int
  private static int getIntBEPartial(final byte[] bytes, final int offset, final int nBytes) {
    int res = switch(nBytes) {
      case 3 -> ((bytes[offset + 1] & 0xFF) << 16)
        | ((bytes[offset + 2] & 0xFF) << 8)
        | (bytes[offset + 3] & 0xFF);
      case 2 -> ((bytes[offset + 2] & 0xFF) << 8)
        | (bytes[offset + 3] & 0xFF);
      case 1 -> (bytes[offset + 3] & 0xFF);
      default -> 0;
    };
    return res;
  }
